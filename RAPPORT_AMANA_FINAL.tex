...
\begin{center}
\flushright \includegraphics[width=4cm]{isigk.png}
\end{center}
\end{minipage}
\end{figure}
...
Au-delà de ces quatre services, Amana se présente comme une plateforme \textit{intelligente}~: l'application Client intègre un assistant conversationnel capable de conseiller le client et de lui proposer des plats, ainsi qu'un moteur de recherche capable d'interpréter une demande en langage naturel ou une simple photo d'un plat pour retrouver les articles correspondants dans le catalogue. Ces deux fonctionnalités, détaillées à la section~\ref{sec:ia} et présentées dans leur usage concret au chapitre~4, reposent sur l'intelligence artificielle plutôt que sur une recherche classique par mots-clés exacts.
...
Ce mémoire est organisé en cinq chapitres. Le premier chapitre pose le cadre du projet~: le contexte, les services proposés, l'analyse des solutions déjà présentes sur le marché tunisien, et le cahier des charges qui en découle. Le deuxième chapitre est consacré à l'analyse et à la conception du système, à travers les diagrammes UML de cas d'utilisation, de classes et de séquence. Le troisième chapitre détaille la réalisation technique~: les technologies utilisées, l'organisation du code, l'implémentation du back-end, la sécurité et l'intégration de l'intelligence artificielle. Le quatrième chapitre présente concrètement les quatre applications et leurs interfaces. Le cinquième et dernier chapitre décrit les tests effectués, l'état du déploiement et les pistes de maintenance envisagées. Une conclusion générale referme le document.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textwidth,keepaspectratio]{illlustration de service de commandee.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{achat des produits depuis un super marché.png}
    \caption{Illustration du service d'achat de produits en supermarché}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{livraison de colis entre particulier.png}
    \caption{Illustration du service de livraison de colis entre particuliers (P2P)}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{livraison de colis entre particulier 2.png}
    \caption{Illustration du service de livraison de colis entre particuliers (P2P)}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{livraison de colis entre particulier 3.png}
    \caption{Illustration du service de livraison de colis entre particuliers (P2P)}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{livraison de colis entre particulier 4.png}
    \caption{Illustration du service de livraison de colis entre particuliers (P2P)}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{paiement de facture.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth,keepaspectratio]{paiement de facture2.png}
    \caption{Illustration du service de paiement de factures}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.5\textwidth,keepaspectratio]{yassir_interface_1.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.5\textwidth,keepaspectratio]{yassir_interface_2.png}}
    \caption{suite~\cite{yassir_site}}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.5\textwidth,keepaspectratio]{livrina_interface.png}}
...
L'innovation d'Amana tient d'abord à la combinaison de services qui, ailleurs, sont proposés séparément. Le règlement de factures via l'application fait gagner du temps aux utilisateurs, et le service de livraison entre particuliers (P2P) permet l'envoi sécurisé d'objets personnels grâce au réseau de livreurs déjà mobilisé pour les commandes classiques.

Elle tient aussi à l'intégration de l'intelligence artificielle, absente des cinq plateformes étudiées à la section~1.4. Plutôt que d'obliger le client à parcourir des menus pour trouver un plat qui correspond à une envie ou à un besoin précis (un repas peu coûteux, adapté à un régime, ou simplement reconnu sur une photo), Amana propose un assistant conversationnel et une recherche intelligente capables de comprendre une demande formulée en langage naturel, en français, en arabe ou en dialecte tunisien. Cette couche d'intelligence artificielle, conçue comme un service transversal accessible aux applications via une fonction serveur dédiée (section~2.5.5), ne remplace aucun des quatre services de base~: elle en simplifie l'accès.
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{cas_utilisation_client_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{cas_utilisation_partenaire_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{cas_utilisation_livreur_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{cas_utilisation_admin_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{raffinement_passer_commande_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{raffinement_suivre_commande_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{raffinement_livraison_commande.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{raffinement_ajout_plat_jour.png}}
...
\subsection{Conception de l'intelligence artificielle}
\label{sec:conception_ia}

Deux fonctionnalités de l'application Client reposent sur l'intelligence artificielle~: l'assistant conversationnel et la recherche intelligente, déjà annoncés dans l'introduction générale et repris comme argument de différenciation à la section~1.6. Sur le plan de la conception, ces deux fonctionnalités posent une contrainte différente de celles vues jusqu'ici~: leur résultat dépend d'un modèle de langage hébergé chez un fournisseur externe, un composant que la plateforme ne maîtrise ni ne contrôle, contrairement à la base de données ou aux Edge Functions internes.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=\textwidth,keepaspectratio]{diagramme_architecture_ia.png}}
    \caption{Architecture conceptuelle de l'intelligence artificielle dans Amana}
    \label{fig:archi-ia-conception}
\end{figure}

Ce constat a guidé un choix de conception simple~: ni l'application Client, ni aucune des trois autres applications, ne communique jamais directement avec un fournisseur d'intelligence artificielle. Une fonction serveur dédiée s'intercale systématiquement entre l'application et le fournisseur, pour trois raisons~:

\begin{itemize}
    \item \textbf{Sécurité~:} la clé d'accès au fournisseur d'IA n'est jamais présente sur le téléphone du client, où elle pourrait être extraite d'une application installée~;
    \item \textbf{Remplaçabilité~:} le fournisseur effectivement utilisé peut changer sans modifier les applications mobiles, qui ne connaissent que l'adresse de la fonction serveur~;
    \item \textbf{Point de contrôle unique~:} la fonction serveur est le seul endroit où surveiller, limiter ou corriger le comportement de cette brique, plutôt que de dupliquer cette logique dans quatre applications.
\end{itemize}

Cette couche d'intelligence artificielle est donc conçue comme un service transversal, au même titre que l'authentification ou les notifications~: elle est accessible depuis l'application Client, mais implémentée une seule fois, côté serveur. Le chapitre~3 (section~3.7) détaille le fonctionnement interne de cette fonction serveur~: le format d'échange, le fournisseur retenu, et le mécanisme de secours mis en place lorsque ce fournisseur ne répond pas.
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=17cm,height=24cm,keepaspectratio]{diagramme_sequence_creation_commande_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=17cm,height=21cm,keepaspectratio]{diagramme_sequence_livraison_commande_v2.png}}
...
\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=15cm,height=21cm,keepaspectratio]{diagramme_sequence_ajout_plat_jour_v2.png}}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{visual image.jpg}
    \caption{Environnement de développement — Visual Studio Code}
...
PostgreSQL combine plusieurs règles définies sur la même table par un \textsc{ou} logique~: il suffit qu'une seule règle autorise l'accès pour qu'il soit accordé, même si toutes les autres l'interdisent. Une relecture de sécurité effectuée en fin de projet a révélé qu'une règle héritée d'une version antérieure du système, devenue inutile, autorisait encore la lecture complète de la table des lignes de commande à n'importe quel appelant, y compris non connecté, en plus des règles correctement limitées au client et au partenaire concernés. Cette règle superflue annulait de fait la restriction voulue. Elle a été supprimée, et remplacée par une règle supplémentaire propre au livreur, pour que celui-ci continue de voir le détail des articles des commandes qui lui sont proposées ou attribuées, sans rouvrir un accès plus large que nécessaire. Cet incident illustre l'attention particulière que demande la combinaison de plusieurs règles RLS sur une même table.
...
Lorsqu'une commande passe au statut confirmée, le serveur cherche automatiquement un livreur disponible dans un rayon d'environ 7~km autour du point de collecte, et lui envoie une offre avec un délai d'environ 30~secondes pour l'accepter ou la refuser. Si le livreur ne répond pas dans ce délai, ou refuse explicitement, l'offre est retirée et proposée au livreur disponible suivant dans le même rayon~: c'est le mécanisme de \textit{dispatch en cascade} déjà évoqué au chapitre~2. Ce mécanisme est mis en œuvre par une fonction stockée côté base de données, appelée à la fois lors de la confirmation de la commande et lors de l'expiration d'une offre non traitée.

Ce rayon de 7~km reste volontairement réduit, la fraîcheur des repas imposant une attribution rapide à un livreur proche. Il arrive toutefois qu'aucun livreur ne soit disponible dans ce périmètre, en particulier en dehors des heures de forte activité. Un mécanisme de repli a donc été ajouté~: lorsque la recherche à 7~km ne trouve aucun livreur éligible, une seconde recherche est automatiquement lancée sur un rayon plus large, avant que le commerçant ne soit averti qu'aucun livreur n'est disponible. Ce rayon élargi est stocké dans une table de paramètres modifiable par l'administrateur plutôt que codé en dur dans l'application, pour pouvoir être ajusté sans nouvelle publication (extrait de code en annexe). Le livreur sollicité par ce repli voit clairement, sur l'écran d'offre, que la course proposée est plus éloignée que d'habitude, avec la distance réelle jusqu'au point de collecte affichée à côté du montant, afin de pouvoir décider en connaissance de cause plutôt que de découvrir la distance après avoir accepté.

Pendant la livraison, la position GPS du livreur est mise à jour dans la base à chaque déplacement d'environ 30~mètres, ce qui permet au client de suivre sa commande en direct, comme décrit au chapitre~2.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{application clientt.png}
    \caption{Logo — application Client}
    \label{fig:client_auth2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight,keepaspectratio]{consultation des restaurants.png}
    \caption{Liste des restaurants disponibles — application Client}
    \label{fig:client_restaurant_liste1}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight,keepaspectratio]{consultation des restaurants2.png}
    \caption{Liste des restaurants disponibles, avec filtrage par catégorie — application Client}
    \label{fig:client_restaurant_liste2}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Exemple de détail d'un restaurant et de son menu — application Client}
...
Un badge « Nouveau » est par ailleurs affiché automatiquement sur la fiche d'un restaurant ajouté à la plateforme depuis moins de sept jours, pour aider le client à repérer les commerces récemment arrivés. Cette règle est purement calendaire~: elle se déclenche pour tout restaurant ajouté, sans intervention manuelle, et s'applique donc automatiquement aux prochains commerces partenaires rejoignant la plateforme après la rédaction de ce mémoire, même si aucun restaurant du jeu de données actuel n'a moins d'une semaine d'ancienneté au moment des tests.
...
Ces deux écrans concentrent l'essentiel des interactions du client avant la confirmation finale, qui déclenche l'écriture en base décrite dans le diagramme de séquence de la section~2.7.2. Une estimation de l'heure d'arrivée est également calculée à ce stade, à partir de la distance entre le commerçant et l'adresse choisie, pour donner au client une idée du délai avant même de valider sa commande~; elle reste une estimation de départ, distincte du temps affiché plus précisément une fois un livreur en route, décrit à la section suivante.


\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight,keepaspectratio]{mon panier.png}
    \caption{Le panier — application Client}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight,keepaspectratio]{paiement et choisi d'adresse.png}
    \caption{Choix de l'adresse et du mode de paiement — application Client}
    \label{fig:client_paiement}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight,keepaspectratio]{client_assistant_ia.png}
    \caption{Assistant conversationnel « Amana Assistant » — application Client}
    \label{fig:client_assistant2}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight,keepaspectratio]{assistant et recherche intelligent.png}
    \caption{Assistant conversationnel « Amana Assistant » — application Client}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{reponse du recherche AI.png}
    \caption{Recherche intelligente par texte ou par photo — application Client}
    \label{fig:client_recherche2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{recherche par photo.png}
    \caption{Recherche intelligente par photo}
    \label{fig:client_recherche3}
\end{figure}
...
\subsection{Recommande rapide et évaluation post-livraison}

Deux fonctionnalités ont été ajoutées vers la fin du projet pour compléter l'historique des commandes du client, sans capture d'écran dédiée disponible au moment de la rédaction de ce mémoire.

La première permet de recommander en un geste une commande déjà livrée~: depuis l'historique, un bouton « Commander à nouveau » recompose le panier avec les mêmes articles, au prix et à la disponibilité du jour, plutôt qu'en rejouant à l'identique le prix figé au moment de l'ancienne commande. Un article devenu indisponible entre-temps, ou dont le commerçant a fermé, est signalé au client plutôt que d'être ajouté silencieusement au panier.

La seconde invite le client à évaluer sa commande, sur cinq étoiles avec un commentaire facultatif, dès que le statut de la commande passe à livrée pendant qu'il consulte l'écran de suivi. Cette évaluation n'est possible qu'une seule fois par commande et alimente automatiquement la note moyenne affichée sur la fiche du restaurant concerné.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{sign up livreur.png}
    \caption{Inscription — application Livreur}
    \label{fig:livreur_signup}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{gestion de la disponiblité de livreur.png}
    \caption{Gestion de la disponibilité — application Livreur}
    \label{fig:livreur_disponibilite}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{consultation des comandes disponibles pour livreur.png}
    \caption{Consultation des commandes disponibles pour le livreur}
    \label{fig:livreur_commandes_dispo}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{nouvelle comande pour livreur.png}
    \caption{Notification d'une nouvelle offre de commande — application Livreur}
    \label{fig:livreur_nouvelle_commande}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{suivi  1.png}
    \caption{Suivi de la livraison sur la carte Mapbox — départ du livreur}
    \label{fig:livreur_suivi1}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{suivi 2.png}
    \caption{Suivi de la livraison sur la carte Mapbox — trajet en cours}
    \label{fig:livreur_suivi2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{suivi 3.png}
    \caption{Suivi de la livraison sur la carte Mapbox — approche de la destination}
    \label{fig:livreur_suivi3}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{atteinte de livraison.png}
    \caption{Arrivée du livreur à destination — application Livreur}
    \label{fig:livreur_arrivee}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{revenus livreur.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{gestion menu partner.png}
    \caption{Gestion du catalogue — application Partenaire}
    \label{fig:partner_menu}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{ajout et modification d'un article.png}
    \caption{Ajout et modification d'un article — application Partenaire}
    \label{fig:partner_article1}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{ajout et modification 2.png}
    \caption{Suite du formulaire d'ajout et modification d'un article}
    \label{fig:partner_article2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{ajout d'un menu complet.png}
    \caption{Ajout d'un menu complet par la fonctionnalité du scan }
    \label{fig:partner_menu_scan}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{gestion des comandes recus.png}
    \caption{Gestion des commandes reçues — application Partenaire}
    \label{fig:partner_commandes}
\end{figure}

\subsection{Gestion du plat du jour}

Le partenaire dispose d'un écran dédié pour mettre en avant un plat du jour, selon le mécanisme décrit à la section~2.4.5~: recherche d'un plat existant, puis association ou création, avant validation.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{gestion plat du jour.png}
    \caption{Recherche et association d'un plat du jour — application Partenaire}
    \label{fig:partner_plat_jour}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{reports partner.png}
    \caption{Statistiques du commerçant — application Partenaire}
    \label{fig:partner_reports}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{payout info.png}
    \caption{Détail des commissions et versements — application Partenaire}
    \label{fig:partner_payout}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth ,keepaspectratio]{tableau admin.png}
    \caption{Tableau de bord principal — Administrateur}
...
Cette section permet de consulter les profils des clients et leur historique de commandes, et d'intervenir sur un compte si nécessaire, dans le respect des règles RLS décrites au chapitre~3. Elle est détaillée pour les clients à la section~4.5.3, tandis que la gestion des livreurs fait l'objet d'une section dédiée (section~4.5.5), en raison des informations spécifiques qu'elle regroupe (position en direct, solde du portefeuille).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,height=0.6\textheight,keepaspectratio]{reste du gestion restaurants.png}
    \caption{Suite de la gestion des restaurants — Administrateur}
    \label{fig:admin_restaurants2}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{gestion des supermarchés.png}
    \caption{Gestion des supermarchés — Administrateur}
    \label{fig:admin_supermarches}
\end{figure}

\subsection{Gestion des clients}
Le tableau de bord administrateur propose une page dédiée à la gestion
des comptes clients. Cette page affiche la liste complète des clients
inscrits sur la plateforme, avec pour chacun son nom, son numéro de
téléphone, son adresse e-mail, le nombre total de commandes passées,
le montant total dépensé, ainsi que la date d'inscription. Un badge
coloré indique en un coup d'œil si le compte est actif ou bloqué. Une
barre de recherche permet à l'administrateur de retrouver rapidement
un client précis par son nom, son numéro ou son e-mail, ce qui devient
utile à mesure que la base de clients grandit.

Pour chaque client, deux actions rapides sont disponibles directement
depuis la liste : un accès à la messagerie pour le contacter, et une
option pour copier ses informations. Un bouton permet également de
bloquer un compte en un seul clic, par exemple en cas de comportement
abusif ou de commandes frauduleuses, sans avoir à supprimer
définitivement le compte du client concerné.


\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{gestion des client dans admin.png}
    \caption{Gestion des clients — Administrateur}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{gestion des livreur dans admin.png}
    \caption{Gestion des livreurs — Administrateur}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{gestion des finances.png}
    \caption{Gestion des finances — Administrateur}
...
Les chapitres précédents ont montré comment la plateforme Amana a été conçue, puis réalisée, puis présentée écran par écran. Ce chapitre est différent~: il ne montre plus à quoi ressemble l'application, il vérifie qu'elle fonctionne vraiment. Pour chaque test, on explique ce qu'on voulait vérifier, ce qu'on a fait exactement, ce qu'on attendait comme résultat, et ce qu'on a réellement observé, capture d'écran à l'appui. Les tests ont été faits à la main, sur des téléphones réels, et non avec des tests automatisés~: ce choix est expliqué à la section~5.6.
...
Vérifier que les trois façons de se connecter (créer un compte, se connecter avec un compte existant, se connecter avec Google) fonctionnent bien, et que le téléphone ne demande un numéro de téléphone que si le client ne l'a pas encore donné.

\paragraph{Scénario / étapes suivies.}
Sur un téléphone Android réel, on a essayé les trois chemins l'un après l'autre~: création d'un nouveau compte, déconnexion puis reconnexion avec ce compte, et enfin connexion directe avec un compte Google. Après chaque essai, on est allé voir directement dans la base de données si un profil avait bien été créé.


\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{authentification par google dans app partner.png}
    \caption{Connexion via Google — application Partenaire}
    \label{fig:test_auth_google_partner}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{exemple de comptes connecté dans la base de donnés.png}
    \caption{Exemple de comptes créés dans la base de données}
    \label{fig:test_auth_comptes_bdd}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{les dates des comptes crées.png}
    \caption{Exemple des dates des comptes créés}
    \label{fig:test_auth_dates_bdd}
\end{figure}
...
Un profil doit être créé à chaque fois, quel que soit le chemin utilisé. L'écran qui demande le numéro de téléphone ne doit apparaître que si ce numéro manque vraiment dans le profil.


\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{numero obligatoire dans partner.png}
    \caption{Numéro obligatoire à la création d'un compte — application Partenaire}
    \label{fig:test_auth_num_partner}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{num obli a la creation d'une comande dans app client.png}
    \caption{Numéro obligatoire à la création d'une commande — application Client}
    \label{fig:test_auth_num_client}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{num obli a la creation de compte dans app driver.png}
    \caption{Numéro obligatoire à la création d'un compte — application Livreur}
    \label{fig:test_auth_num_driver}
\end{figure}
...
Les trois façons de se connecter ont marché. Un profil a bien été créé en base à chaque essai, et l'écran de saisie du numéro de téléphone n'est apparu que pour les comptes qui n'en avaient pas encore, notamment après une connexion Google. La figure~\ref{fig:test_auth} montre l'écran par lequel ces trois chemins commencent~: on y voit les deux onglets « Se connecter » et « S'inscrire », les champs email et mot de passe, et le bouton de connexion Google en bas.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{client_auth.png}
    \caption{Écran de connexion utilisé pour le test d'authentification}
    \label{fig:test_auth}
\end{figure}
...
\paragraph{Objectif du test.}
Vérifier que le parcours de commande enregistre bien une commande correcte en base de données, avec le bon montant, et que la réduction de fidélité s'applique automatiquement au bon moment, sans que le client ait besoin de faire quoi que ce soit de spécial.

\paragraph{Scénario / étapes suivies.}
On a suivi le parcours complet~: choix des articles, ajout au panier, choix de l'adresse et du mode de paiement, puis validation. Pour tester la fidélité, on a utilisé un compte client dont on connaissait déjà le nombre de commandes passées, et on lui a fait passer volontairement une commande qui tombe sur un palier de récompense (la cinquième commande, qui donne droit à une livraison à moitié prix). On a comparé le montant affiché au client avec le montant attendu, puis on est allé vérifier en base de données le compteur de commandes du client et le montant réellement enregistré.



\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{reduction 50.png}
    \caption{Réduction de moitié des frais de livraison pour la 5\textsuperscript{e} commande}
    \label{fig:test_fidelite_reduction50}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{5 eme comande.png}
    \caption{La 5\textsuperscript{e} commande dans la carte de fidélité}
    \label{fig:test_fidelite_5eme}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{livraison gratuite.png}
    \caption{Livraison gratuite pour la 10\textsuperscript{e} commande}
    \label{fig:test_fidelite_gratuite}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{fidilité 10.png}
    \caption{La 10\textsuperscript{e} commande dans la carte de fidélité}
    \label{fig:test_fidelite_10eme}
\end{figure}
...
\paragraph{Résultat attendu.}
La commande doit être enregistrée avec le bon statut de départ, et le bon montant total. Pour la cinquième commande d'un client, les frais de livraison doivent déjà apparaître réduits de moitié au moment où le client valide sa commande, avant même la confirmation~: la réduction ne doit pas être ajoutée après coup.

\paragraph{Résultat obtenu.}
La commande a été correctement enregistrée en base, avec le bon statut et le bon montant. Pour le test de fidélité, la réduction a bien été appliquée dès la création de la commande, avant que le client ne voie le total final, ce qui confirme que le calcul se fait côté serveur au moment où la commande est enregistrée. La figure~\ref{fig:test_commande} montre l'écran de paiement utilisé pendant ce test~: on y voit l'adresse de livraison, le mode de paiement choisi, et le montant final que le client doit payer, affiché juste avant qu'il ne confirme sa commande.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{paiement et choisi d'adresse.png}
    \caption{Écran de paiement utilisé pour vérifier le montant final avant confirmation}
...
\paragraph{Objectif du test.}
Vérifier que le système propose bien une commande à un seul livreur à la fois, que ce livreur a un temps limité pour répondre, et que la commande est proposée au livreur suivant s'il ne répond pas à temps. Vérifier aussi que l'alerte reçue par le livreur est fiable.

\paragraph{Scénario / étapes suivies.}
Ce test a demandé deux téléphones~: un pour le client, un pour le livreur. On a créé une commande puis on l'a marquée comme prête côté commerçant, en notant l'heure exacte de ce changement. On a ensuite mesuré le temps qu'il a fallu pour que la notification apparaisse sur le téléphone du livreur. Le test a été refait plusieurs fois~: livreur disponible, livreur indisponible, application du livreur ouverte, puis application complètement fermée.

\paragraph{Résultat attendu.}
La notification doit apparaître rapidement sur le téléphone du livreur, avec un son d'alerte qui doit sonner même si le téléphone est en mode silencieux. Le livreur a un temps limité pour accepter ou refuser~; passé ce délai, la commande doit être automatiquement proposée à un autre livreur disponible.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{notification livreur.png}
    \caption{Notification pour acceptation d'une commande pendant 30 secondes}
    \label{fig:test_livraison_notif}
\end{figure}
...
\paragraph{Résultat obtenu.}
Le système a bien proposé la commande à un seul livreur à la fois, avec un délai mesuré d'environ une à deux secondes entre le changement de statut en base et l'affichage de la notification. La figure~\ref{fig:test_livraison} montre la notification reçue par le livreur~: on y voit le titre « Nouvelle commande ! », le détail de la commande, le montant total, et les deux boutons « Refuser » et « Accepter ». Ce test a aussi fait ressortir deux problèmes réels, découverts et corrigés pendant les essais.



\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{nouvelle comande pour livreur.png}
    \caption{Notification d'une nouvelle commande reçue par le livreur}
...
\paragraph{Objectif du test.}
Vérifier que la position du livreur, affichée sur la carte du client, se met bien à jour pendant la livraison, et que l'écran ne reste jamais bloqué même en cas de petit problème de connexion.

\paragraph{Scénario / étapes suivies.}
Un téléphone jouant le rôle du livreur a été déplacé physiquement pendant qu'une commande était en cours de livraison, en gardant l'écran de suivi du client ouvert en même temps. Le test a été fait une première fois avec une connexion internet stable, puis une seconde fois en coupant volontairement la connexion par moments, pour voir comment l'application réagit dans un cas moins favorable.

\paragraph{Résultat attendu.}
Le point représentant le livreur doit bouger sur la carte du client au fur et à mesure de son déplacement, et le statut affiché doit changer à chaque étape confirmée par le livreur. Une coupure de connexion ponctuelle ne doit pas bloquer l'affichage pour le reste de la livraison.

\paragraph{Résultat obtenu.}
Le suivi a globalement bien fonctionné, mais le test avec coupures de connexion a montré un cas où le point du livreur restait bloqué sur la carte après un seul échec de mise à jour, sans aucun message d'erreur visible pour le client. On a corrigé ce problème en faisant en sorte qu'un échec ponctuel n'empêche plus les mises à jour suivantes de fonctionner. Un nouveau test, avec les mêmes coupures volontaires, a confirmé que le point du livreur reprenait bien ses mises à jour après un incident. La figure~\ref{fig:test_suivi} montre l'écran de suivi utilisé pendant ce test~: on y voit le statut de la commande (ici « Pending ») et les quatre étapes de la livraison, du côté droit de l'écran.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{client_suivi_commande.png}
    \caption{Écran de suivi de commande utilisé pour le test en temps réel}
...
\paragraph{Objectif du test.}
Vérifier qu'une demande de colis entre particuliers démarre bien avec le
bon statut, qu'elle est proposée en même temps à tous les livreurs
disponibles plutôt qu'à un seul, et qu'elle ne peut jamais être acceptée
par deux livreurs à la fois.
...
\paragraph{Scénario et étapes suivies.}
Une demande de colis a été créée depuis l'application client. Deux comptes
livreurs distincts, connectés simultanément sur deux téléphones et tous
deux disponibles, ont ensuite observé l'arrivée de cette demande en temps
réel, puis tenté de l'accepter au même moment afin de vérifier qu'un seul
d'entre eux pouvait effectivement l'obtenir.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{comande colis pour livreur 1.png}
    \caption{Notification de la demande de colis reçue par le premier livreur}
    \label{fig:test_colis1}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{comande colis our livreur 2.png}
    \caption{Notification de la même demande de colis reçue simultanément par le second livreur}
    \label{fig:test_colis2}
\end{figure}



\paragraph{Résultat attendu.}
Un colis ne nécessite aucune préparation, contrairement à une commande de
repas : il doit donc apparaître directement avec le statut « prêt ».
Puisqu'il n'y a pas de restaurant à privilégier par la distance, la demande
doit être envoyée en même temps à l'ensemble des livreurs disponibles, et
non l'un après l'autre. Le premier à l'accepter doit réussir, le second
doit recevoir un message clair l'informant que la demande n'est plus
disponible. Si aucun livreur disponible n'accepte dans un délai raisonnable,
la demande doit ensuite être proposée aux livreurs actuellement en cours de
livraison, afin qu'elle ne reste pas sans réponse.


\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{comande prete directement.png}
    \caption{Statut initial « prêt » de la demande de colis, sans étape de préparation}
    \label{fig:test_colis_pret}
\end{figure}
...
\paragraph{Résultat obtenu.}
Le statut initial du colis est conforme à ce qui était attendu : la ligne
correspondante est créée directement avec le statut « prêt » en base de
données, sans passer par l'étape intermédiaire utilisée pour les commandes
de repas. Le mécanisme empêchant la double acceptation a également été
validé~: une seule mise à jour réussit à associer un livreur au colis, la
seconde tentative étant rejetée directement par la base de données.

L'analyse du fonctionnement initial a toutefois révélé plusieurs anomalies
qui n'apparaissent pas sur les commandes de repas. D'une part, la fonction
chargée d'envoyer l'alerte lisait les coordonnées de récupération sous un
nom de champ différent de celui utilisé pour les enregistrer, ce qui
l'amenait à tort à conclure qu'aucune position n'était disponible et à ne
prévenir aucun livreur. D'autre part, le mécanisme d'envoi ne diffusait pas
la demande à tous les livreurs disponibles en même temps comme prévu, mais
les sollicitait l'un après l'autre avec un délai de 15 secondes entre
chacun, un comportement hérité du dispatch des commandes de repas et non
adapté à ce cas.

Ces deux anomalies ont été corrigées. La fonction lit désormais le bon nom
de champ, et la demande est envoyée simultanément à tous les livreurs
disponibles dans un rayon de 7~km. Un système de priorité en deux temps a
également été mis en place~: la demande est d'abord proposée uniquement aux
livreurs sans course en cours, et n'est étendue aux livreurs déjà occupés
qu'au bout de 30~secondes sans réponse. Le message reçu par le livreur
n'ayant pas obtenu le colis a par ailleurs été traduit en français, afin de
rester cohérent avec le reste de l'interface.

\textbf{Remarque~:} nous n'avons pas de capture d'écran dédiée à ce test
dans les fichiers actuels du rapport. Une capture montrant la réception
simultanée de l'alerte sur les deux téléphones, ainsi que le message reçu
par le livreur n'ayant pas obtenu le colis, reste à réaliser.
\subsection{Test de l'assistant intelligent}

\paragraph{Objectif du test.}
Vérifier que l'assistant conversationnel comprend bien la langue du client dès le premier message, et surtout, que ses réponses se traduisent bien par des plats affichés à l'écran, et pas seulement par un texte qui promet des résultats sans rien montrer.



\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{assistant tounsi.png}
    \caption{Échange avec l'assistant conversationnel en dialecte tunisien}
    \label{fig:test_assistant_tounsi}
\end{figure}
...
\paragraph{Scénario / étapes suivies.}
On a envoyé plusieurs messages à l'assistant, en français, en arabe et en dialecte tunisien, dont des demandes liées à un objectif de santé, par exemple « je fais du sport, propose-moi des plats riches en protéines ». Après chaque réponse, on a vérifié non seulement le texte renvoyé par l'assistant, mais aussi si des plats étaient réellement affichés en dessous. On a aussi testé une demande pour un plat qui n'existe pas du tout sur la plateforme, comme des sushis, pour voir ce que fait l'assistant quand il ne trouve vraiment rien.
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{assistant francais.png}
    \caption{L'assistant répond en français et conseille des plats adaptés à un objectif nutritionnel}
    \label{fig:test_assistant_francais}
\end{figure}


\paragraph{Résultat attendu.}
La langue utilisée dans le premier message doit être gardée pendant toute la conversation. Pour une demande liée à la santé, l'assistant doit donner un petit conseil, puis afficher une liste de plats qui existent vraiment sur la plateforme et qui correspondent à la demande. Quand aucun plat ne correspond, l'utilisateur doit recevoir un message clair, plutôt que de ne rien voir du tout.

\paragraph{Résultat obtenu.}
La détection de la langue a bien fonctionné dans tous les essais. La figure~\ref{fig:test_assistant} montre un échange réel en dialecte tunisien~: le client écrit « aaslema », l'assistant répond dans la même langue, puis le client demande « nheb pizza », et l'assistant répond avec deux plats affichés juste en dessous, chacun avec un bouton « Commander ». Ce test confirme que l'assistant répond bien dans la bonne langue et affiche de vrais plats, pas seulement un texte.

En revanche, les tests avec des demandes liées à la santé ont montré un vrai problème~: pour une demande comme celle sur les protéines, l'assistant donnait un bon conseil nutritionnel, mais la liste de plats en dessous était vide, sans aucune explication. En regardant le fonctionnement de l'assistant, on a compris pourquoi~: il choisissait un seul mot précis pour chercher des plats (par exemple « poulet »), et ce mot ne correspondait presque jamais au texte réel des plats dans le catalogue. On a corrigé cela en ajoutant, en plus de ce mot précis, une liste plus large de mots liés à l'objectif de santé. Après cette correction, un nouveau test avec la même demande a bien renvoyé une liste de plats. Le test avec la demande de sushis a aussi confirmé qu'un message clair s'affiche maintenant quand aucun plat ne correspond, plutôt que de laisser l'écran vide sans explication. \textbf{Remarque~:} aucune capture d'écran de ce cas précis (demande liée aux protéines, avant correction) n'a été conservée~; la figure~\ref{fig:test_assistant} montre le fonctionnement général de l'assistant (langue détectée, plats affichés), mais pas cet incident particulier.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.5\textheight,keepaspectratio]{assistant et recherche intelligent.png}
    \caption{Échange réel avec l'assistant conversationnel, en dialecte tunisien, avec des plats affichés}
    \label{fig:test_assistant}
\end{figure}

Enfin, ces tests ont aussi permis de confirmer, comme expliqué au chapitre~3, que la clé du fournisseur principal (OpenRouter) ne fonctionne pas en pratique, et que presque toutes les réponses observées viennent en réalité du fournisseur de secours, Google Gemini.

\section{Tests des interfaces}

Les interfaces des trois applications mobiles ont été testées à la main sur plusieurs tailles d'écran Android, pour vérifier que les listes, les cartes et les formulaires restent lisibles, sans texte qui dépasse de l'écran. Le tableau de bord Administrateur a été testé dans un navigateur sur ordinateur, puisque c'est sa cible principale~; il n'a pas été testé spécifiquement sur des petits écrans.

\section{Gestion des erreurs}

Plusieurs mécanismes ont été mis en place pour éviter qu'une panne ponctuelle ne bloque complètement une fonctionnalité. Le changement automatique de fournisseur d'intelligence artificielle, expliqué au chapitre~3, en est le meilleur exemple~: si le fournisseur principal ne répond pas, un second fournisseur prend le relais sans que l'utilisateur s'en aperçoive. De la même façon, si le classement avancé de la recherche intelligente échoue, un simple tri par prix croissant est utilisé à la place, pour toujours renvoyer une réponse plutôt que de laisser l'utilisateur sans rien. Le test de l'assistant intelligent, décrit plus haut, a aussi conduit à ajouter un message clair quand une recherche ne renvoie vraiment aucun résultat, au lieu de laisser l'écran vide.

\section{Déploiement de l'application}

Au moment de la rédaction de ce rapport, la plateforme Amana est déployée dans un cadre de test~: le serveur Supabase est en ligne et accessible en permanence, ce qui a permis de faire tous les tests décrits plus haut dans des conditions réelles. Les trois applications mobiles ont été installées à la main sur des téléphones Android pour ces tests, mais elles ne sont pas encore publiées sur les magasins d'applications officiels (Google Play, App Store). Le tableau de bord Administrateur a été testé en local et n'a pas encore été mis en ligne sur un hébergeur public. La publication officielle et la mise en ligne du tableau de bord restent des étapes à faire après la soutenance de ce projet.

\section{Maintenance et évolutions possibles}

Plusieurs pistes d'amélioration ont été identifiées pendant le projet, certaines déjà mentionnées au chapitre~3~:

\begin{itemize}
    \item \textbf{Tests automatisés~:} remplacer une partie des tests manuels décrits dans ce chapitre par des tests automatiques, pour sécuriser les futures modifications du code.
    \item \textbf{Limiter le nombre d'appels à l'intelligence artificielle~:} ajouter une limite par utilisateur pour l'assistant et la recherche intelligente, afin de mieux maîtriser le coût payé aux fournisseurs.
    \item \textbf{Mettre en cache les réponses de recherche~:} éviter de payer deux fois le même appel au modèle de langage.
    \item \textbf{Recherche par sens plutôt que par mots-clés~:} dans la continuité directe de la correction faite après le test de l'assistant intelligent, pour mieux comprendre des demandes formulées différemment.
    \item \textbf{Notifications plus fiables en arrière-plan~:} chercher d'autres solutions face à la limite observée sur certains téléphones Android à interface MIUI.
    \item \textbf{Élargir la zone couverte~:} étendre le service au-delà du gouvernorat de Kairouan, une fois la phase pilote validée.
\end{itemize}

\section{Conclusion}

Ce chapitre a montré que la plateforme Amana a été testée à la main, sur des situations réelles~: connexion, commande, livraison, suivi en direct, envoi de colis, et assistant intelligent. Pour chaque test, la même méthode a été suivie~: un objectif clair, un scénario précis, un résultat attendu, et une comparaison honnête avec ce qui s'est réellement passé. Cette méthode a permis non seulement de vérifier que le système fonctionne, mais aussi de trouver et de corriger de vrais problèmes pendant les tests, comme le montrent le cas du point du livreur qui restait bloqué et celui de la recherche liée aux objectifs de santé. Elle a aussi permis de dire honnêtement quelles limites n'ont pas pu être corrigées dans le temps du projet, en particulier l'absence de tests automatisés et la limite connue des notifications sur certains téléphones Android. L'application est donc, pour l'instant, encore dans une phase de test avant une publication officielle. Les pistes de maintenance présentées ici préparent les évolutions possibles reprises dans la conclusion générale de ce mémoire.
...
Le résultat de ce travail est un système de quatre applications reliées à un backend commun~: une application Client qui réunit la commande de repas, l'achat de produits, l'envoi de colis entre particuliers et le paiement de factures, enrichie d'un assistant conversationnel, d'une recherche intelligente, d'un programme de fidélité, d'une fonctionnalité de recommande rapide et d'une évaluation post-livraison~; une application Livreur organisée autour d'un mécanisme de dispatch en cascade à rayon adaptatif et d'un suivi GPS en temps réel~; une application Partenaire pour la gestion du catalogue et des commandes reçues~; et un tableau de bord Administrateur pour superviser l'ensemble de la plateforme, ses utilisateurs et ses finances.

\subsection*{Apports professionnels du projet}

Au-delà du résultat technique, ce projet a permis de manipuler des problématiques concrètes de systèmes distribués~: la synchronisation en temps réel entre plusieurs applications, la sécurité des données par les règles RLS, la gestion d'un service externe pouvant tomber en panne, ou encore la conception d'un mécanisme d'attribution automatique tolérant aux refus et aux absences de réponse. Le choix d'une démarche itérative a également permis de développer une capacité à ajuster une conception initiale face à des contraintes découvertes en cours de route, plutôt que de suivre un plan figé du début à la fin~; les corrections apportées au mécanisme de dispatch et à une règle de sécurité de la base de données peu avant le dépôt de ce mémoire, décrites au chapitre~3, en sont un exemple concret.
...
Cette section regroupe des captures d'écran complémentaires qui ne trouvaient pas naturellement leur place dans le corps du chapitre~4, notamment pour les fonctionnalités ajoutées en fin de projet (recommande rapide, évaluation post-livraison, mise en avant des nouveaux restaurants), qui ne disposent pas encore de capture dédiée au moment de la rédaction de ce mémoire.
...
\subsection*{Recherche du prochain livreur éligible, avec rayon adaptatif (SQL)}

Référencée à la section~3.5.4. Cette fonction recherche d'abord un livreur dans le rayon normal de 7~km~; si aucun livreur n'y est disponible, elle élargit une seule fois la recherche à un rayon plus large avant d'abandonner. Le rayon élargi est stocké dans une table de paramètres modifiable, plutôt que codé en dur, pour rester ajustable sans nouvelle version de l'application.

\begin{verbatim}
v_driver_id := next_eligible_driver(p_order_id, p_radius_km);

IF v_driver_id IS NULL THEN
  -- Aucun livreur dans le rayon normal : on elargit une seule fois
  v_driver_id := next_eligible_driver(
    p_order_id, driver_fallback_radius_km()
  );
END IF;
\end{verbatim}
