...
- [ ] **~20 food items priced in millimes instead of dinars** (discovered 2026-08-04, while debugging AI-search zero-results): of 302 available `food_items`, 242 are priced in dinars (`3`, `9.9`, `23.8`) but ~20 are stored in millimes (`Eau minérale` = `700`, `CAPPUCCINO` = `2000`, `DIRECT` = `1300`) — mostly `Boissons`/`Drinks` rows. Mixed units in one column silently break every budget comparison: the AI-search intent parser turns "moins de 10 dinars" into `price <= 10`, which excludes a 2 DT coffee stored as `2000`, and the same applies to any future price filter or sort. Fix is a data normalisation (divide the millime rows by 1000) — needs a per-row check first, since there is no flag distinguishing the two units beyond magnitude. **Not fixed** — flagged only, per 2026-08-04 decision. Related: the zero-priced-items entry above; note that an independent count on 2026-08-04 found **40** available rows at `price = 0`, not 19, so that item's scope may be wider than recorded.
...
- ~~**Logout bug**: profile-screen logout doesn't call `signOut()` (Google session + Supabase) and never clears/unregisters the FCM token → next account on same device can receive the previous user's pushes.~~ **Fixed, 2026-09-12** (driver app, `lib/features/auth/data/auth_repository.dart`): `signOut()` now deletes this device's `device_tokens` row (before `auth.signOut()`, while `auth.uid()` still resolves for RLS), then calls `_googleSignIn.signOut()` and `_supabase.auth.signOut()`. **Same-session follow-up**: found and fixed the identical class of bug in the background GPS isolate — `background_location_service.dart` persists the tracking driver's id to SharedPreferences (`bg_driver_id`/`bg_delivery_id`) completely separately from the Supabase session, so without an explicit stop call a stale isolate kept pushing GPS and ringing the delivery-offer alarm under the *previous* account after a different one signed in on the same physical device (confirmed live on a real Honor device while investigating an unrelated alert-sound issue). `signOut()` now also calls `BackgroundLocationService.stopTracking()`. Neither fix committed yet as of this note.
...
- ~~**Reorder feature** (customer re-orders a past order) — not started.~~ **Built, 2026-09-15** (mobile): `OrderRepository.getReorderItems()` re-fetches each past food order's items at *current* price/availability (skips deleted/unavailable items or ones whose restaurant is now closed, reporting the skip count) rather than replaying the stale order snapshot; "Commander à nouveau" button on delivered food orders in `order_history_screen.dart` clears the cart (confirming first if it isn't empty) and pushes the reordered items in. Food orders only — no supermarket reorder entry point. Not committed yet as of this note.
- ~~**Post-delivery rating flow** — not started.~~ **Built, 2026-09-15** (mobile + DB): new `order_ratings` table (migration `20260915090000_order_ratings.sql`) — 1-5 stars + optional comment, write-once (no update/delete policy), RLS INSERT check re-derives eligibility from `orders` itself (own order, delivered, food, restaurant_id matches) rather than trusting client input, `AFTER INSERT` trigger recomputes `restaurants.rating`/`review_count` as a full aggregate. `RatingPromptSheet.maybeShow()` (mirrors `LoyaltyCardSheet`'s pattern) is scheduled once an order's live status becomes `delivered` in `order_tracking_screen.dart`, gated on `OrderRepository.hasRating()` so it doesn't re-ask after a rating exists — but dismissing without rating is untracked, so the prompt can reappear on reopening that same order later (accepted trade-off for a minimal v1). Not committed yet as of this note.
